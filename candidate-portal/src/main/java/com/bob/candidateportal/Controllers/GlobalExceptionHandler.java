...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> handleAllExceptions(Exception ex, WebRequest request) {
        ...
        logger.error("Unhandled exception occurred in candidate portal: {}", ex.getMessage(), ex);
        logger.debug("Request details: {}", requestDetails);
        
        String errorMessage = "An unexpected error occurred: " + ex.getMessage();
        ApiResponse<Object> response = new ApiResponse<>(false, errorMessage, ex.getLocalizedMessage());
    ...
    public ResponseEntity<ApiResponse<Map<String, String>>> handleValidationExceptions(MethodArgumentNotValidException ex, WebRequest request) {
        ...
        logger.warn("Validation failed in candidate portal: {}", validationErrors);
        logger.debug("Request details: {}", getRequestDetails(request));
        
        ApiResponse<Map<String, String>> response = new ApiResponse<>(false, "Validation failed. Please check the entered details.", errors);
